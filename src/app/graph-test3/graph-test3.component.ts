...
export class GraphTest3Component implements OnInit {
  ...
  private zoom: any;
  private clusterNodeSize = { width: 40, height: 20 };
    ...
    return d3.zoom()
      ...
      .on("zoom", (event: any) => {
        ...
        if (currentZoomScale >= 0.4 && !this.expandedCluster) {
          ...
          if (nearestNode.data.txid.includes('cluster')) {
            ...
            this[nodes].forEach((d: any) => {
              if (d.data.txid.includes('txo') && !visited) {
                if (d.parent.data.txid === nearestNodeParent.parent.data.txid && d.data.vout === nearestNodeParent.data.vout) {
          ...
          this[nodes].forEach((d: any) => {
            if (d.data.txid.includes('txo') && !visited) {
              if (d.parent.data.txid === originalParent.parent.data.txid && d.data.vout === originalParent.data.vout) {
  ...
  updateTree(source:any, side: string) {
    ...
    this[nodes].forEach((d: any) => {
      d.y = (side === 'origin' ? -1 : 1) * 300 * d.depth;
    ...
    const transactionNodesUpdate = nodeUpdate.filter(function (d: any) {
        return !d.data.txid.includes('cluster') && !(d.data.txid === 'utxo' || d.data.txid === 'stxo');
    });
    ...
    txoNodesUpdate
      ...
      .text(function(d: any) {
        return d.data.txid === 'utxo' ? 'Unspent' : 'Spent';
      })
      .append("tspan")
      .attr("dy", "1.2em")
      .attr("x", 0)
